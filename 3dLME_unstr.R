   ...
   if (!all(abs(inData) < 10e-8)) {        
      ...
      if(lop$ML) {
         if(!is.na(lop$corStr[1]))
            try(fm <- lme(ModelForm, random = lop$ranEffList, dataframe, correlation=corSymm(form=(~1|Subj)), control=lmeControl(returnObject=TRUE), method='ML'), silent=TRUE)
         ...
         if(!is.na(lop$corStr[1]))
            try(fm <- lme(ModelForm, random = lop$ranEffList, dataframe, correlation=corSymm(form=(~1|Subj)), control=lmeControl(returnObject=TRUE)), silent=TRUE)
         # case of basis functions
         if(is.null(fm)) { # case of basis functions fails or other cases
...
if(lop$ICC) {  # ICC part
   ...
   while(is.null(fm)) {
      ...
      if(!is.na(lop$corStr[1])) try(fm <- lme(ModelForm, random=lop$ranEffList, data=lop$dataStr, 
         correlation=corSymm(form=(~1|Subj)), control=lmeControl(returnObject=TRUE)), silent=TRUE) else try(fm <- lme(ModelForm, 
